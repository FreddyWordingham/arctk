...
use crate::{display_field_ln, render, AspectRatio, Build, Error, Pos3, X, Y};
use attr::load;
use std::{
...
impl Build for Camera {
    ...
    fn build(self, _in_dir: &Path) -> Result<Self::Inst, Error> {
        ...
        let focus = render::Focus::new(self.pos, self.tar, dof);
        ...
        let lens = render::Lens::new(
            [swivel[X].to_radians(), swivel[Y].to_radians()],
            self.hr_fov.to_radians(),
        );
        let sensor = render::Sensor::new(&self.aspect, self.hr_res, self.ss_power);
