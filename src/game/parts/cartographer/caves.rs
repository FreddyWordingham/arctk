...
/// Construct a set of cave map.
#[inline]
#[must_use]
pub fn caves(rng: &mut RandomNumberGenerator) -> Map {
    // Filled map.
    ...
    for _ in 0..MAX_ROOMS {
        ...
        if rooms.iter().all(|r| !r.intersect(&cave)) {
            ...
            super::set_zone(&mut tiles, &interior, Tile::Floor);
            rooms.push(cave);
        }
    }
    rooms.shrink_to_fit();
    ...
    for (room_a, room_b) in rooms.iter().zip(rooms.iter().skip(1)) {
        ...
        if rng.range(0, 2) == 0 {
            super::set_path(&mut tiles, start, end, Tile::Floor);
        } else {
            super::set_path_inv(&mut tiles, start, end, Tile::Floor);
        }
    }

    Map::new(tiles, rooms[0].center())
}
