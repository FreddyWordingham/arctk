...
use crate::{cartographer::Input, Dir3, Group, Pos3, Ray, Trace, Vec3};
use attr::load;
...
impl Cast {
    ...
    pub fn observe_mat(&self, input: &Input, pos: Pos3) -> Option<Group> {
        match self {
            ...
            Self::Radiant(samples) => {
                ...
                for n in 0..*samples {
                    let (pitch, roll) = crate::sample::golden::sphere(n as i32, *samples as i32);
    ...
    pub fn see_mat(input: &Input, ray: Ray) -> Option<Group> {
        ...
        if let Some(hit) = input
            .tree
            .observe(ray.clone(), input.sett.bump_dist(), bound_dist)
        {
